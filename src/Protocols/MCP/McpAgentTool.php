...
final class McpAgentTool extends AbstractGovernedTool
{
    public function __construct(
        ...
        private readonly ?McpServerPolicy $serverPolicy=null,
    ){}
    ...
    public function sideEffects():array{return ['remote_mcp_call'];}
    public function execute(AgentContext $context,array $arguments):ToolResult
    {
        $this->serverPolicy?->assertAllowed((string)$context->tenantId,$this->serverName,$this->toolName);
        $data=$this->clients->get($this->serverName)->callTool($this->toolName,$arguments,['tenant_id'=>$context->tenantId,'actor_id'=>$context->actorId,'correlation_id'=>$context->correlationId]);
        return ToolResult::success($data,evidence:['mcp:'.$this->serverName.':'.$this->toolName]);
    }
}
